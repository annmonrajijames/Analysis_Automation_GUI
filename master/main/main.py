...
def open_file_folder():
    global folder_path
    choice = messagebox.askquestion("Select type", "Do you want to select a file?")
    if choice == "yes":
...
def copy_folder():
    ...
    if destination_path:
        if os.path.isdir(folder_path):
            ...
            destination_folder = destination_path  # Set destination folder to the chosen directory
            shutil.copy(folder_path, destination_folder)  # Copy file to the chosen directory
...
def reset_gui():
    ...
    path_label.config(text="Click Choose File/Folder - Path will be shown here")
...
path_label = tk.Label(app, text="Click Choose File/Folder - Path will be shown here", bg="lightblue", fg="black", wraplength=100)
path_label.grid(row=1, column=0, padx=10, pady=10, sticky='e')

# Button to choose the folder
folder_button = tk.Button(app, text="Choose File/Folder", command=open_file_folder, bg="lightblue", fg="black")
